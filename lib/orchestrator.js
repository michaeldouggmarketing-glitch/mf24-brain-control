...
export async function orchestrate(text, {
  ...
  const economy = await aiCaller({layer:'economy_ai', text:raw, today, local, global, apiKey});
  ...
  try {
    const advanced = await aiCaller({layer:'advanced_ai', text:raw, today, local:economyResult, global, apiKey});
