import { needsFinancialIntelligence, hasCompleteMonetaryCoverage } from "./financial-coverage.js";
...
function privateStatus(context, state = "unavailable") {
  ...
  return {
    ...
    financial_snapshot: Boolean(context.financial_snapshot),
    recent_transactions: Array.isArray(context.financial_snapshot?.recent_transactions)
      ? context.financial_snapshot.recent_transactions.length
      : 0,
  ...
  try {
    ...
    if (typeof requestId === "string" && requestId) {
      ...
      await call("mf24_brain_record_usage", {
        ...
        p_input_tokens: Number.isFinite(Number(usage.input_tokens))
          ? Number(usage.input_tokens)
          : null,
        p_output_tokens: Number.isFinite(Number(usage.output_tokens))
          ? Number(usage.output_tokens)
          : null,
        p_cached_input_tokens: Number.isFinite(Number(usage.input_tokens_details?.cached_tokens))
          ? Number(usage.input_tokens_details.cached_tokens)
          : null,
        p_estimated_cost_usd: Number.isFinite(Number(output.estimated_cost_usd))
          ? Number(output.estimated_cost_usd)
          : null,
    ...
    return error.message === "brain_service_role_not_configured"
      ? "pending_service_role"
      : "failed";
  }
}
...
export async function orchestrate(
  ...
  if (!raw || raw.length > 4000)
    throw Object.assign(new Error("text_required_or_too_long"), { status: 400 });

  const local = nativeInterpreter(raw, { today });
  const requiresAi = needsFinancialIntelligence(raw);
  const completeOrClarify = (value) => {
    if (
      value.intent !== "create_transactions" ||
      hasCompleteMonetaryCoverage(raw, value.transactions)
    )
      return value;
    return {
      ...value,
      ...
      confidence: 0,
      reply:
        "Não consegui representar todos os lançamentos com segurança. Confirme os valores e as datas; nada foi registrado.",
    };
  };

  let privateContext = null;
  let privateContextState = mf24UserId ? "unavailable" : "not_requested";
  if (mf24UserId && mf24SpaceId) {
    try {
      privateContext = await privateContextLoader({
        userId: mf24UserId,
        spaceId: mf24SpaceId || undefined,
        today,
      });
      ...
      privateContextState =
        error.message === "mf24_prod_service_role_not_configured"
          ? "not_configured"
  ...
  if (
    !requiresAi &&
    safeNumber(local.confidence, 0) >= 0.9 &&
    local.resolution_layer !== "economy_ai"
  ) {
    return {
  ...
  if (
    !requiresAi &&
    global &&
    ["deterministic", "global_knowledge"].includes(globalLayer) &&
    globalConfidence >= 0.92 &&
    local.transactions?.length
  ) {
    return {
      ...normalise(
        { ...local, confidence: Math.max(safeNumber(local.confidence, 0), globalConfidence) },
        globalLayer,
      ),
      private_memory: contextStatus,
      global_knowledge: compactGlobal(global),
  ...
  if (!apiKey) {
    return {
      ...completeOrClarify(normalise(local, "economy_ai")),
      private_memory: contextStatus,
      global_knowledge: compactGlobal(global),
  ...
  const economyTelemetry = await recordAiUsage({
    text: raw,
    channel,
    call: rpcCaller,
    output: economy,
  ...
  const needsAdvanced =
    economy.result?.needs_advanced === true ||
    economyResult.confidence < (requiresAi ? 0.85 : 0.65) ||
    (economyResult.intent === "create_transactions" &&
      !hasCompleteMonetaryCoverage(raw, economyResult.transactions));

  if (!needsAdvanced) {
    return {
      ...completeOrClarify(economyResult),
      private_memory: contextStatus,
      global_knowledge: compactGlobal(global),
      openai: {
        used: true,
        ...
        model: economy.model,
        usage: economy.usage,
        estimated_cost_usd: economy.estimated_cost_usd,
        stored: false,
      },
  ...
  try {
    ...
    const advancedTelemetry = await recordAiUsage({
      text: raw,
      channel,
      call: rpcCaller,
      output: advanced,
    ...
    return {
      ...completeOrClarify(normalise(advanced.result, "advanced_ai")),
      private_memory: contextStatus,
      global_knowledge: compactGlobal(global),
      openai: {
        used: true,
        ...
        estimated_cost_usd: advanced.estimated_cost_usd,
        stored: false,
        escalated_from: "economy_ai",
      },
    ...
    return {
      ...
      openai: {
        ...
        model: economy.model,
        stored: false,
        advanced_fallback_error: "advanced_ai_unavailable",
      },
      telemetry: economyTelemetry,
    };
  }
